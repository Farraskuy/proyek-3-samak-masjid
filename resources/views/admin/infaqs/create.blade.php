...
@section('content')
    <section class="p-3">
        <form action="{{ route('admin.infaqs.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h4 class="fw-semibold mb-0">Tambah Program Infaq</h4>
                </div>
                    ...
                    <div class="card bg-white border-0 rounded-3 p-4 mb-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control"
                                placeholder="Contoh: Infaq Pembangunan Masjid" value="{{ old('name') }}" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-semibold">Deskripsi</label>
                            <textarea name="description" class="form-control" rows="6" placeholder="Jelaskan detail program infaq ini...">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </div>
                            ...
                            <select name="bank_account_id" class="form-select" required>
                                ...
                                @foreach ($banks as $bank)
                                    <option value="{{ $bank->account_id }}"
                                        {{ old('bank_account_id') == $bank->account_id ? 'selected' : '' }}>
                                        {{ $bank->bank_name }} - {{ $bank->account_number }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="form-text">Rekening tempat dana infaq ini akan ditampung.</div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="image-uploader-wrapper text-center p-3 border rounded bg-light position-relative"
                                ...
                                <div id="preview-container"
                                    class="d-none h-100 w-100 position-absolute top-0 start-0 bg-white rounded overflow-hidden">
                                    <img id="preview-image" src="" class="w-100 h-100 object-fit-cover">
                                    <button type="button"
    ...
    @push('scripts')
        <script>
            function previewImage(input) {
                if (input.files && input.files[0]) {
                    ...
                    reader.onload = function(e) {
                        ...
                        document.getElementById('placeholder-content').classList.add('opacity-0');
                    }
            ...
            function removeImage() {
                ...
                document.getElementById('placeholder-content').classList.remove('opacity-0');
            }
        </script>
    @endpush
@endsection
