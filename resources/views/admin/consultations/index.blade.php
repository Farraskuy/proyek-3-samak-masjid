        ...
        <div class="row g-0 h-100">
            <div class="col-md-2 col-lg-2 border-end h-100 bg-white d-flex flex-column py-3">
                ...
                <div class="space-y-2 px-3">
                    <a href="{{ route('admin.consultations.index', ['status' => 'all']) }}" class="d-flex align-items-center justify-content-between px-3 py-2 rounded-3 text-decoration-none {{ request('status') == 'all' || !request('status') ? 'bg-dark text-white shadow-sm' : 'text-secondary hover-bg-light' }}">
                        <div class="d-flex align-items-center"><i class="fas fa-inbox me-2" style="width: 20px; text-align: center;"></i><span class="fw-medium">Semua</span></div>
                        <span class="badge {{ request('status') == 'all' || !request('status') ? 'bg-secondary' : 'bg-light text-dark' }} rounded-pill">{{ $counts['all'] ?? 0 }}</span>
                    </a>
                    <a href="{{ route('admin.consultations.index', ['status' => 'pending']) }}" class="d-flex align-items-center justify-content-between px-3 py-2 rounded-3 text-decoration-none {{ request('status') == 'pending' ? 'bg-dark text-white shadow-sm' : 'text-secondary hover-bg-light' }}">
                        <div class="d-flex align-items-center"><i class="fas fa-clock me-2" style="width: 20px; text-align: center;"></i><span class="fw-medium">Pending</span></div>
                        @if (($counts['pending'] ?? 0) > 0) <span class="badge bg-danger rounded-pill">{{ $counts['pending'] ?? 0 }}</span> @else <span class="badge {{ request('status') == 'pending' ? 'bg-secondary' : 'bg-light text-dark' }} rounded-pill">0</span> @endif
                    </a>
                    <a href="{{ route('admin.consultations.index', ['status' => 'active']) }}" class="d-flex align-items-center justify-content-between px-3 py-2 rounded-3 text-decoration-none {{ request('status') == 'active' ? 'bg-dark text-white shadow-sm' : 'text-secondary hover-bg-light' }}">
                        <div class="d-flex align-items-center"><i class="fas fa-comments me-2" style="width: 20px; text-align: center;"></i><span class="fw-medium">Aktif</span></div>
                        <span class="badge {{ request('status') == 'active' ? 'bg-secondary' : 'bg-light text-dark' }} rounded-pill">{{ $counts['active'] ?? 0 }}</span>
                    </a>
                    <a href="{{ route('admin.consultations.index', ['status' => 'closed']) }}" class="d-flex align-items-center justify-content-between px-3 py-2 rounded-3 text-decoration-none {{ request('status') == 'closed' ? 'bg-dark text-white shadow-sm' : 'text-secondary hover-bg-light' }}">
                        <div class="d-flex align-items-center"><i class="fas fa-check-circle me-2" style="width: 20px; text-align: center;"></i><span class="fw-medium">Selesai</span></div>
            ...
            <div class="col-md-4 col-lg-3 border-end h-100 d-flex flex-column bg-white">
                <div class="p-3 border-bottom">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <small class="text-muted fw-bold text-uppercase" style="font-size: 0.75rem; letter-spacing: 0.5px;">{{ request('status') ? ucfirst(request('status')) : 'Semua' }} ({{ $consultations->count() }})</small>
                    </div>
                </div>
                        ...
                        <div class="list-group-item list-group-item-action border-0 p-4 border-bottom cursor-pointer {{ request()->route('id') == $item->id ? 'bg-blue-50' : '' }}" onclick="loadChat({{ $item->id }}, this)">
                            <div class="d-flex align-items-start">
                                <div class="position-relative me-3">
                                    <div class="rounded-circle bg-secondary d-flex align-items-center justify-content-center text-white fw-bold" style="width: 45px; height: 45px; font-size: 1.2rem;">{{ substr($item->user->full_name ?? 'H', 0, 1) }}</div>
                                ...
                                <div class="flex-grow-1">
                                    <div class="d-flex justify-content-between align-items-start mb-1">
                                        <div><h6 class="fw-bold text-dark mb-0">{{ $item->question_from ?? 'Anonim' }}</h6><small class="text-muted" style="font-size: 0.75rem;">{{ $item->user->email ?? '-' }}</small></div>
                                    ...
                                    <div class="d-flex gap-2 mt-2">
                                        <form action="{{ route('admin.consultations.accept', $item->id) }}" method="POST" onsubmit="return confirm('Terima konsultasi ini?')">@csrf<button type="submit" class="btn btn-success btn-sm py-0 px-2" style="font-size: 0.75rem;"><i class="fas fa-check me-1"></i> Terima</button></form>
                                        <button type="button" onclick="openRejectModal({{ $item->id }})" class="btn btn-outline-danger btn-sm py-0 px-2" style="font-size: 0.75rem;"><i class="fas fa-times me-1"></i> Tolak</button>
                                    </div>
                                    @endif
                                </div>
                            </div>
                ...
                <div id="chat-content" class="h-100">
                    ...
                    @else
                        <div class="h-100 d-flex flex-column align-items-center justify-content-center text-center p-5">
                            <div class="mb-4"><div class="bg-white rounded-circle p-4 shadow-sm d-inline-block"><i class="fas fa-comments fa-3x text-primary opacity-50"></i></div></div>
...
<script>
    ...
    function appendMessageToChat(message, user) {
        ...
        if(message.message_type === 'file' && message.attachment_url) {
            ...
            attachmentHtml = `<a href="/${message.attachment_url}" target="_blank" class="${color} text-decoration-none"><i class="fas fa-paperclip me-1"></i> Lampiran</a><br>`;
        }
        ...
        if(confirmBtn){
            confirmBtn.addEventListener('click', function () {
                ...
                .then(data => {
                    ...
                    } else { alert('Gagal menutup konsultasi.'); }
                })
    ...
    document.addEventListener('submit', function(e) {
        if (e.target && e.target.id === 'admin-chat-form') {
            ...
            axios.post(`/admin/konsultasi/${currentConsultationId}/pesan`, formData)
                ...
                .catch(err => { console.error('Error:', err); alert('Gagal mengirim pesan.'); });
        }
    });
