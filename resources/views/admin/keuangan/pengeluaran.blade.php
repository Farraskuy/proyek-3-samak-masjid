    ...
    <section class="p-3">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div class="d-flex align-items-center gap-2">
                ...
                <h4 class="fw-semibold mb-0">Tambah Pengeluaran</h4>
            </div>
        </div>
                        ...
                        <form action="{{ route('admin.keuangan.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-semibold">Bank / Rekening Sumber <span
                                    ...
                                    <select name="bank_name" class="form-select" required>
                                        ...
                                        @foreach ($banks as $bank)
                                            <option value="{{ $bank->bank_name }}"
                                                ...
                                                {{ $bank->bank_name }} - Saldo: Rp
                                                {{ number_format($bank->saldo_saat_ini ?? 0, 0, ',', '.') }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="category" class="form-select" required>
                                        ...
                                        <option value="Operasional"
                                            {{ old('category') == 'Operasional' ? 'selected' : '' }}>Operasional</option>
                                        <option value="Pemeliharaan"
                                            {{ old('category') == 'Pemeliharaan' ? 'selected' : '' }}>Pemeliharaan</option>
                                        <option value="Listrik & Air"
                                            {{ old('category') == 'Listrik & Air' ? 'selected' : '' }}>Listrik & Air
                                        </option>
                                        <option value="Kegiatan" {{ old('category') == 'Kegiatan' ? 'selected' : '' }}>
                                            Kegiatan</option>
                                        <option value="Gaji" {{ old('category') == 'Gaji' ? 'selected' : '' }}>Gaji
                                        </option>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <small class="text-muted">Minimal Rp 1.000 (tidak boleh melebihi saldo bank)</small>
                                </div>
                            </div>
