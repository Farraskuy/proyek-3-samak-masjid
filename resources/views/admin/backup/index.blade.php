            ...
            <div class="col-lg-8">
                ...
                <form action="{{ route('admin.backup.store') }}" method="POST">
                    ...
                    <div class="card bg-white border-0 rounded-3 p-4 mb-4">
                        <h5 class="fw-semibold mb-3"><i class="fas fa-database me-2 text-primary"></i>Buat Backup Manual
                        </h5>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="d-flex gap-4">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="storage_type" value="local"
                                        id="storage_local" checked>
                                    <label class="form-check-label" for="storage_local">
                                        <i class="fas fa-server me-1 text-muted"></i> Storage Server
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="storage_type" value="download"
                                        id="storage_download">
                ...
                <div class="card bg-white border-0 rounded-3 p-4 mb-4">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h5 class="fw-semibold mb-0"><i class="fas fa-clock me-2 text-warning"></i>Jadwal Backup Otomatis
                        </h5>
                    ...
                    <form action="{{ route('admin.backup.schedule.update') }}" method="POST" id="scheduleForm">
                        @csrf
                        <input type="hidden" name="is_enabled" id="isEnabledInput"
                            value="{{ $schedule->is_enabled ? '1' : '0' }}">
                        ...
                        <div id="scheduleSettings" style="display: {{ $schedule->is_enabled ? 'block' : 'none' }};">
                            <div class="row g-3 mb-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="frequency" class="form-select rounded-3">
                                        <option value="daily" {{ $schedule->frequency == 'daily' ? 'selected' : '' }}>
                                            Harian</option>
                                        <option value="weekly" {{ $schedule->frequency == 'weekly' ? 'selected' : '' }}>
                                            Mingguan</option>
                                        <option value="monthly" {{ $schedule->frequency == 'monthly' ? 'selected' : '' }}>
                                            Bulanan</option>
                                        <option value="yearly" {{ $schedule->frequency == 'yearly' ? 'selected' : '' }}>
                                            Tahunan</option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <select name="output_format" class="form-select rounded-3">
                                        <option value="single"
                                            {{ $schedule->output_format == 'single' ? 'selected' : '' }}>Single SQL File
                                        </option>
                                        <option value="zip" {{ $schedule->output_format == 'zip' ? 'selected' : '' }}>
                                            ZIP (Per Tabel)</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <button type="button" class="btn btn-outline-primary btn-sm rounded-3 ms-2"
                                    data-bs-toggle="modal" data-bs-target="#tableSelectModal">
                                    <i class="fas fa-table me-1"></i>Pilih Tabel ({{ count($schedule->tables ?? []) }}
                                    dipilih)
                                </button>
                                ...
                                @if (count($schedule->tables ?? []) > 0)
                                    <div class="mt-2">
                                        <small class="text-muted">Tabel terpilih:
                                            {{ implode(', ', $schedule->tables ?? []) }}</small>
                                    </div>
                                @endif
                            </div>
                ...
                <form action="{{ route('admin.backup.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="card bg-white border-0 rounded-3 p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control rounded-3" name="backup_file" accept=".sql,.txt"
                                required>
                        ...
                        @forelse($backups as $backup)
                            <div class="p-3 mb-2 rounded-3" style="background: #f8f9fa;">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    {{-- Restore --}}
                                    <button type="button" class="btn btn-sm btn-warning rounded-3" title="Restore"
                                        onclick="showConfirmModal({
                                            action: '{{ route('admin.backup.restore', $backup['name']) }}',
                                            method: 'POST',
                                            type: 'restore',
                                            title: 'Restore Database',
                                            message: 'PERINGATAN: Restore akan menjalankan semua SQL dalam file ini. Data mungkin tertimpa. Lanjutkan?',
                                    ...
                                    {{-- Delete --}}
                                    <button type="button"
                                        class="btn btn-sm btn-outline-danger rounded-3 btn-delete-article" title="Hapus"
                                        data-action="{{ route('admin.backup.destroy', $backup['name']) }}">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content rounded-3">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title fw-semibold"><i class="fas fa-table me-2"></i>Pilih Tabel untuk Jadwal Backup
                    </h5>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <button type="button" class="btn btn-sm btn-outline-primary rounded-3 me-1"
                            id="modalSelectAll">Pilih Semua</button>
                        <button type="button" class="btn btn-sm btn-outline-secondary rounded-3"
                            id="modalDeselectAll">Batal Pilih</button>
                    </div>
