...
<section class="py-5 bg-pattern" style="background-color: #175C9E; height: 320px; display:flex; align-items:center;">
    <div class="container text-center">
        <h1 class="display-5 fw-bold text-white mb-3">Barang Hilang & Ditemukan</h1>
    ...
    <div class="container">
        ...
        <div class="d-flex justify-content-center mb-4">
            <form method="GET" class="w-100 d-flex shadow-sm rounded-pill px-3 py-2" style="max-width: 800px;">
                ...
                <input type="text" name="search" class="form-control border-0 shadow-0" placeholder="Cari barang..." value="{{ request('search') }}" style="background:none; flex: 1;">
            ...
            <div class="row g-3 w-100" style="max-width: 800px;">
                <div class="col-md-2 text-center">
                    <a href="{{ route('layanan.barang-hilang') }}?tab=lost" class="text-decoration-none text-dark">
                        <div class="bg-light p-3 rounded-4 mb-2" style="color: #175C9E"><i class="fas fa-th-large fs-2"></i></div>
                        <h6 class="fw-bold">Semua</h6>
                    </a>
                </div>
                @foreach ($categories as $cat)
                <div class="col-md-2 text-center">
                    <a href="{{ route('layanan.barang-hilang') }}?tab=lost&category={{ $cat->slug }}" class="text-decoration-none text-dark">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-9">
                            ...
                            <div class="d-flex flex-wrap gap-3 text-muted small">
                                <span><i class="fas fa-map-marker-alt text-success me-1"></i> {{ $item->location_lost }}</span>
                                <span><i class="fas fa-calendar-alt text-primary me-1"></i> {{ $item->lost_at->format('d M Y') }}</span>
                                <span><i class="fas fa-tag text-warning me-1"></i> {{ $item->category->name }}</span>
                            </div>
                        </div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content rounded-4 shadow-lg border-0">
                    ...
                    <div class="modal-body p-4">
                        ...
                        @else
                        <img src="{{ asset('storage/' . $item->photos->first()->image_url) }}" class="img-fluid rounded-4 mb-3" style="max-height:400px; object-fit:contain;">
                        @endif
                        <h4 class="fw-bold">{{ $item->item_name }}</h4>
                        <p class="text-muted">{{ $item->description }}</p>
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item px-0">
                                <span><i class="fas fa-map-marker-alt text-success me-2"></i> Lokasi Perkiraan Hilang</span>
                                <strong class="float-end">{{ $item->location_lost }}</strong>
                            </li>
                            <li class="list-group-item px-0">
                                <span><i class="fas fa-calendar-alt text-success me-2"></i> Tanggal Hilang</span>
                ...
                <div class="card rounded-4 shadow-sm border-0 item-card p-3">
                    <div class="row align-items-center">
                        <div class="col-md-9">
                            ...
                            <p class="text-muted mb-2">{{ $item->description }}</p>
                            <div class="d-flex flex-wrap gap-3 text-muted small">
                                <span><i class="fas fa-map-marker-alt text-success me-1"></i> {{ $item->location_found }}</span>
                                <span><i class="fas fa-calendar-alt text-primary me-1"></i> {{ $item->created_at->format('d M Y') }}</span>
                                <span><i class="fas fa-tag text-warning me-1"></i> {{ $item->category }}</span>
                                <span>
                        ...
                        <div class="col-md-3 text-md-end mt-3 mt-md-0">
                            <button class="btn btn-outline-primary rounded-pill px-4" data-bs-toggle="modal"
                                data-bs-target="#foundModal{{ $item->item_id }}">
                                Lihat Detail
                            </button>
                        </div>
                    </div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content rounded-4 shadow-lg border-0">
                    ...
                    <div class="modal-body p-4">
                        ...
                        <img src="{{ asset('storage/' . $item->photos[0]->image_url) }}" class="img-fluid rounded-4 mb-3" style="max-height:400px; object-fit:contain;">
                        @else
                        <div id="foundCarousel{{ $item->item_id }}" class="carousel slide">
                            <div class="carousel-inner rounded-4 shadow-sm">
                                @foreach ($item->photos as $index => $photo)
                                <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                    <img src="{{ asset('storage/' . $photo->image_url) }}" class="d-block w-100" style="max-height:400px; object-fit:contain;">
                                </div>
                                @endforeach
                            </div>
                            <button class="carousel-control-prev" type="button" data-bs-target="#foundCarousel{{ $item->item_id }}" data-bs-slide="prev">
                                <span class="carousel-control-prev-icon"></span>
                            </button>
                            <button class="carousel-control-next" type="button" data-bs-target="#foundCarousel{{ $item->item_id }}" data-bs-slide="next">
                        ...
                        <ul class="list-group list-group-flush">
                            ...
                            <li class="list-group-item px-0">
                                ...
                                <strong class="float-end">{{ $item->created_at->format('d M Y H:i') }}</strong>
                            </li>
                            <li class="list-group-item px-0">
