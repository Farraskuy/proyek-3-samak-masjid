...
function refreshKonfirmasiLink() {
    ...
    const zakatSectionVisible =
        document.getElementById("section-zakat").style.display !== "none";
    ...
    const selectedBankEl = document.querySelector(
        'input[name="selected_bank"]:checked'
    );
...
function renderZakatForm() {
    ...
    if (type === "maal") {
        ...
        html = `<label class="form-label">Jumlah Emas (Gram)</label>
                    ...
                    <small class="text-muted">Asumsi harga emas: ${formatRupiah(
                        HARGA_EMAS
                    )} / gram</small>`;
...
function showRekening(categoryType) {
    ...
    const filtered = window.BankAccounts.filter(
        (acc) => acc.category === "all" || acc.category === categoryType
    );
    ...
    if (filtered.length > 0) {
            ...
            html += `
                    <label class="d-block mb-3" style="cursor:pointer">
                        <input type="radio" name="selected_bank" 
                               ...
                               onchange="refreshKonfirmasiLink()" ${
                                   isChecked ? "checked" : ""
                               }>

                        <div class="bank-card custom-checkbox-label"> 
                        <img src="${acc.logo_url}" alt="${
                acc.bank_name
            }" class="bank-logo">
                        <div class="flex-grow-1">
                            <h6 class="mb-0 fw-bold">${acc.bank_name}</h6>
                            <div class="d-flex align-items-center">
                                <span class="me-2 fs-5" id="rek-${
                                    acc.account_id
                                }">${acc.account_number}</span>
                                <i class="fas fa-clipboard copy-btn" onclick="event.preventDefault(); copyText('rek-${
                                    acc.account_id
                                }')" title="Salin"></i>
                            </div>
        ...
        container.innerHTML =
            '<p class="text-center text-muted">Belum ada rekening yang tersedia.</p>';
        wrapper.style.display = "block";
    }
}
