...
class WebsiteInformationController extends Controller
{
    ...
    {
        $page = WebsiteInformation::firstOrFail();

        // Process images in content
        $content_html = $this->processContentImages($page->content);

        return view('client.website-information.about-us', [
            'page' => $page,
     ...
     */
    /**
     * Admin: Display "Informasi Website" dashboard (Singleton)
     */
    public function indexAdmin(Request $request)
    {
        $page = WebsiteInformation::first();

        if ($page) {
            $page->content = preg_replace(
                '/<img\s+[^>]*src="(website-information\/[^"]+)"/i',
                '<img src="/storage/$1"',
                $page->content
            );
        }

        return view('admin.website-information.index', compact('page'));
    }
    ...
    {
        $page = WebsiteInformation::findOrFail($id);
        ...
        return view('admin.website-information.edit', compact('page'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            'title' => 'required|string|max:255|unique:website_information,title,' . $id,
        ...
        if ($request->hasFile('featured_image_url')) {
            ...
            $validated['featured_image_url'] = $image->storeAs('website-information/images', $newName, 'public');
        }
        ...
        if ($content) {
            ...
            $content = preg_replace(
                '/src="\/storage\/(website-information\/[^"]+)"/i',
                'src="$1"',
                $content
            );
        ...
        try {
            ...
            return redirect()->route('admin.website-information.index')
    ...
    public function create()
    {
        ...
        $count = WebsiteInformation::count();

        // Jika sudah ada 1 atau lebih balikin ke sebelumnya yahahaha
        if ($count >= 1) {
            return redirect()->route('admin.website-information.index')
                ->with('error', 'Maksimal hanya boleh ada 1 Halaman Informasi Website. Silakan edit halaman yang sudah ada.');
        }

        // Jika belum ada data, tampilkan view create tanpa mengirim variabel $page
        return view('admin.website-information.tambah_website-information');
    }
    ...
    public function store(Request $request)
    {
        ...
        if (WebsiteInformation::count() >= 1) {
            return redirect()->route('admin.website-information.index')
                ->with('error', 'Kuota halaman informasi website sudah penuh.');
        }

        // 2. Validasi Input
        $validated = $request->validate([
            'title' => 'required|string|max:255|unique:website_information,title',
            'description' => 'nullable|string|max:500',
            'content' => 'required|string',
        ...
        DB::beginTransaction();
        try {
            WebsiteInformation::create($validated);

            DB::commit();

            return redirect()->route('admin.website-information.index')
                ->with('success', 'Informasi website berhasil dibuat!');
    ...
    private function deleteRemovedQuillImages($oldContent, $newContent)
    {
        ...
        // Regex disesuaikan dengan folder: website-information/
        preg_match_all('/<img[^>]+src="(website-information\/[^"]+)"/i', $oldContent, $oldMatches);
        $oldImages = $oldMatches[1] ?? [];

        // 2. Ambil semua path IMG dari konten BARU (Input User yang sudah diproses)
        preg_match_all('/<img[^>]+src="(website-information\/[^"]+)"/i', $newContent, $newMatches);
